...
def operations(maxima_filtered,filter_matrix,fft_datatemp,width,height,limit):
    ...
    limit = 40000
    for window_size in range(20,300):
        window_size=window_size*2
        
        x_before = width_window-window_size//2 #calulate x values before
        x_after = width_window+window_size//2#calulate x values after
         
        y_before = height_window-window_size//2 #calulate y values before
        y_after = height_window+window_size//2#calulate y values after


        newval=np.mean(abs(fft_datatemp[y_before:y_after,x_before:x_after]))

        # plt.plot(window_size//2,newval,'o',color="red")

        
        if oldval-newval<limit:       
            break
        oldval = newval
    ...
    return y_before, y_after , x_before, x_after , width_window , height_window,window_size
...
def fft_filter(fft_datatemp,filter_matrix,limit,filterwindow):
    ...
    maxima_filtered = ndimage.maximum_filter(abs(fft_datatemp_cut),50, mode='constant')#apply the maxima filter

    
    y_before, y_after, x_before,x_after,width_window, height_window,window_size = operations(maxima_filtered,filter_matrix,fft_datatemp,width,height,limit)#do the operations method
    ...
    window_matrix=np.pad(hamming, [(y_before,y_after), (x_before,x_after )], mode='constant')#pad the window to the size of the image
    
    windowed_data_orig = np.multiply(window_matrix,fft_datatemp)#multiply so that only the peaks where the window is is kept

    
    if filterwindow:
        return abs(windowed_data_orig)#returns if the printing condition is the filtered window
    

    windowed_data = np.roll(windowed_data_orig, int(abs(width_window-width//2)), axis=1)#roll back to the center in x direction
    windowed_data = np.roll(windowed_data, int(-abs(height_window-height//2)), axis=0)#roll back to the center in y direction
