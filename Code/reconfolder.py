...
from NAMES import ControlPanel_name

from Plugin import Plugin
from PyQt5.QtWidgets import (QWidget,QPushButton,
                             QHBoxLayout, QLabel, QComboBox,QSpinBox,QGridLayout,QRadioButton,QLineEdit,QFileDialog)
...
class reconfolder(QWidget,Plugin):
    ...
    def setUp_launchButton(self): 
        ...
        """
        self.pluginDict[ControlPanel_name].widget().addTab(self.interactionDisplay(), self.name)
    ...
    def startrecon(self):
        ...
        self.thread1 = threading.Thread(target=self.folderwalk, args=[self.savePath,self.loadPath])
        self.condition = True
    ...
    def folderwalk(self,savelocation,loadlocation):
        ...
        for (subdir, dirs, files)in os.walk(loadlocation): 
            ...
            for file in files:
                ...
                printimage,window_size = mainfile.run(image,self.window_size,self.FFT,self.FFT_filter,self.auto)#reconstruct image
