...
from NAMES import ControlPanel_name, Display_name

from Plugin import Plugin
from PyQt5.QtWidgets import (QWidget,QPushButton,QGridLayout,
                             QHBoxLayout, QLabel, QComboBox,QSpinBox,QRadioButton)
...
class reconstruction(QWidget,Plugin):
    ...
    def setUp_launchButton(self): 
        ...
        """
        self.pluginDict[ControlPanel_name].widget().addTab(self.manualinteractionDisplay(), self.name)
        currentIndex = self.pluginDict[ControlPanel_name].widget().currentIndex()
        if currentIndex != 1:
            self.pluginDict[ControlPanel_name].widget().setCurrentIndex(currentIndex + 1)
    ...
    def Getimage(self):
        try:
            bridge = Bridge()#creates the transfer layer
    
            core = bridge.get_core() #an object that is the core of micromanager
            ...
            self.newimage = np.asarray(tagged_image.pix.reshape((image_height, image_width)))#reshapes the image
        except:
            print("No connection to Micro-manager found")
            self.newimage = self.oldimage
        # pathtoimage = r"E:\remotedesk\data\sperm\21062023_spermcells_healthy_15msecexp_40X_661nm\set2\I2_17.tif"
        # self.newimage = np.array(Image.open(pathtoimage))
    def single_shot(self):
        ...
        Numpyimage,window_size = mainfile.run(self.newimage,self.window_size,self.FFT,self.FFT_filter,self.auto)#reconstruct image
        ...
        currentWidget.UpdateImage(Numpyimage)
