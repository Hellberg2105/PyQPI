...
def tilt_removal(image,advanced):
    ...
    if advanced == True: #checks if the tilt correction is advanced or not
        ...
        for x in range(1,samples): #loop through samples
            ...
            y1 = image[xshape-x*xshape//samples,:] #calculates the vectors given sample
    ...
    x_correction = np.asmatrix(np.fromfunction(p,(len(y1),)))#calculates the x direction correction
    y_correction = np.asmatrix(np.fromfunction(p2,(len(y2),)))#calculates the y direction correction
